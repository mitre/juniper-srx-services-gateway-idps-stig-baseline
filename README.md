# Juniper SRX Services Gateway Intrusion Detection and Prevention System (IDPS) Security Technical Implementation Guide
This InSpec Profile was created to facilitate testing and auditing of `Juniper SRX Services Gateway IDPS`
...
- Benchmark Version: **Version 2 Release 1 (V2R1)**
...
The Juniper SRX Services Gateway IDPS STIG Profile uses the [InSpec](https://github.com/inspec/inspec)
...
Table of Contents
=================
- [Juniper SRX Services Gateway Intrusion Detection and Prevention System (IDPS) Security Technical Implementation Guide](#juniper-srx-services-gateway-intrusion-detection-and-prevention-system-idps-security-technical-implementation-guide)
...
Requirements associated with the Juniper SRX Services Gateway IDPS STIG are derived from the
...
The Juniper SRX Services Gateway IDPS STIG profile checks were developed to provide technical implementation
...
```yaml
  ...
  # used by the Juniper SRX Services Gateway IDPS STIG profile.
...
bundle exec [inspec or cinc-auditor] exec https://github.com/mitre/juniper-srx-services-gateway-idps-stig-baseline.git --input-file=<your_inputs_file.yml> -t juniper://username@device.example.com --password 'secret' --reporter=cli json:<your_results_file.json>
...
mkdir profiles
cd profiles
git clone https://github.com/mitre/juniper-srx-services-gateway-idps-stig-baseline.git
bundle exec [inspec or cinc-auditor] archive juniper-srx-services-gateway-idps-stig-baseline

# Using `juniper` transport without bastion host
bundle exec [inspec or cinc-auditor] exec <name of generated archive> --input-file=<your_inputs_file.yml> -t juniper://username@device.example.com --password 'secret' --reporter=cli json:<your_results_file.json>
...
```bash
cd juniper-srx-services-gateway-idps-stig-baseline
git pull
cd ..
bundle exec [inspec or cinc-auditor] archive juniper-srx-services-gateway-idps-stig-baseline --overwrite
